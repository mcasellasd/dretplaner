...
const INPUT_FILE = path.join(DATA_DIR, 'fiscalitat_directa_activitats_chunks.json');
...
async function main() {
    ...
    for (const doc of documents) {
        count++;
        if (count % 50 === 0) {
            console.log(`Processant document fiscalitat ${count}/${documents.length}...`);
        }
        ...
        points.push({
            ...
            payload: {
                text: doc.text,
                source: 'fiscalitat_directa_activitats_chunks.json',
                chunk_index: doc.id,
                source_pdf: doc.meta?.source_pdf,
