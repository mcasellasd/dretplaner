...
const INPUT_FILE = path.join(DATA_DIR, 'irpf_chunks.json');
...
async function main() {
    ...
    for (const doc of documents) {
        count++;
        if (count % 50 === 0) {
            console.log(`Processant document IRPF ${count}/${documents.length}...`);
        }
    ...
    try {
        ...
        console.log("Pujant informació de la llei IRPF a Qdrant...");
        ...
        console.log("\n✅ Procés finalitzat amb èxit! El fitxer IRPF JSON ha estat incrustat i integrat a la base de coneixement.");
